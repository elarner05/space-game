...
void ChunkLoader::update() {
    ...
    for (const ChunkCoord& coord : desiredChunks) {
        if (!m_loadedChunks.count(coord)) {
            loadChunk(coord);
            m_loadedChunks.insert(coord); // missing!
        }
    }
}
...
void ChunkLoader::unloadChunk(ChunkCoord coord) {
    ...
    file.write(reinterpret_cast<const char*>(&ChunkLoader::CHUNK_FORMAT_VERSION), sizeof(ChunkLoader::CHUNK_FORMAT_VERSION));

    uint32_t count = 0; // count only persistent entities
    for (EntityID id : it->second) {
        size_t idx = Core::entityIndexTable[id.value];
        if (!((uint8_t)Core::entityFlagTable[idx] & (uint8_t)EntityFlags::Persistent))
            count++;
    }
    if (count == 0) {
        // no need to save an empty chunk, just delete the file as we created it ^
        deleteChunkFile(coord);
        // still need to remove non-persistent entities from chunkMap
        while (true) {
    ...
    for (EntityID id : it->second) {
        size_t idx = Core::entityIndexTable[id.value];
        if ((uint8_t)(Core::entityFlagTable[idx]) & (uint8_t)(EntityFlags::Persistent)) continue;
    ...
    // Core::unregisterEntity modifies chunkMap so iterate until empty
    while (true) {
        auto it2 = Core::chunkMap.find(coord);
        if (it2 == Core::chunkMap.end() || it2->second.empty()) break;
        EntityID front = it2->second.front();
        size_t idx = Core::entityIndexTable[front.value];
        if ((uint8_t)(Core::entityFlagTable[idx]) & (uint8_t)(EntityFlags::Persistent)) {
            // remove from list but don't unregister
            it2->second.erase(it2->second.begin());
            continue;
        }
