...
bool Core::setEntityChunk(Kinematics& kin, EntityID id, ChunkCoord newChunk) {
    // directly set chunk without modifying local position (used for teleportation)
    ChunkCoord oldChunk = kin.chunk;
    if (oldChunk == newChunk) return false;

    kin.chunk = newChunk;
    
    // here onwards is copied from resolveEntityChunk, could be refactored to avoid duplication
    // remove from old chunk
    auto it = Core::chunkMap.find(oldChunk);
    if (it == Core::chunkMap.end()) {
        assert(false && "oldChunk missing in chunkMap");
